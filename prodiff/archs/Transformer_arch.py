...
from prodiff.archs.utils.Involution_Kernel_Abs_Pos import Invo_TransformerBlock
...
class Conv2DMod(nn.Module):
    def __init__(self, in_chan, out_chan, kernel, style_ch=81, demod=True, stride=1, padding=1, dilation=1, eps=1e-8,
                 **kwargs):
        super().__init__()
...
class MDnet(nn.Module):
    def __init__(self, dim_in, dim_out, expansion_rate=4, shrinkage_rate=0.25, dropout=0.):
        super().__init__()
...
##############################
## Scale-aware Prompt Block ##
##############################
class Scale_aware_Prompt_Block(nn.Module):
    def __init__(self,
                 dim_in,
                 dim_out,
                 *,
        ...
        self.prompt = PromptGenBlock(prompt_dim=dim_in, prompt_len=5, prompt_size=32, lin_dim=self.h_dim)
        self.cat_conv = nn.Conv2d(hidden_dim * 2, hidden_dim, 3, 1, 1)
    ...
    def forward(self, x, x_size, scale):
        ...
        input = input + self.net2(self.norm1(input).transpose(1, 2).view(b, self.h_dim, h, w), scale).flatten(
            2).transpose(1, 2)
        ...
        propmt_v = self.prompt(input)
        input = self.cat_conv(torch.cat([input, propmt_v], dim=1)).flatten(2).transpose(1, 2)
...
class RBF(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        lengthscale = torch.tensor(3.0) if lengthscale \
                                           is None else torch.tensor(lengthscale)
        self.lengthscale = nn.parameter.Parameter(lengthscale)

    #   sigma = torch.tensor(1.0) if sigma \
    #                       is None else torch.tensor(sigma)
...
class INM(nn.Module):
    ...
    def __init__(self, in_dim,
        ...
        self.inv1 = Invo_TransformerBlock(latent_dim, 3, 1, ffn_expansion_factor=1, LayerNorm_type="WithBias",
                                          use_CA=True, attn_type="Invo")
        self.inv2 = Invo_TransformerBlock(latent_dim, 3, 1, ffn_expansion_factor=1, LayerNorm_type="WithBias",
                                          use_CA=True, attn_type="Invo")
    ...
    def forward(self, feat, prior, coord, cell=None):
        B, C, H, W = feat.shape
        ...
        q_feat = F.grid_sample(
            feat, coord, mode='nearest', align_corners=False)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.ln1_1 = nn.Linear(16, 1)
        self.ln1_2 = nn.Linear(16, 1)

    def forward(self, x, prior, b, rpi, mask=None):
        """
        Args:
        ...
        x = x.view(b, c, self.window_size[0], self.window_size[1], -1)

        k1 = self.ln1(prior).permute(0,2,1)
        k1 = self.ln1_1(k1).unsqueeze(-1).unsqueeze(-1)

        k2 = self.ln2(prior).permute(0,2,1)
        k2 = self.ln1_2(k2).unsqueeze(-1).unsqueeze(-1)
...
class atten_block(nn.Module):
    ...
    def forward(self, x, prior, x_size, rpi_sa, attn_mask):
        h, w = x_size
        b, _, c = x.shape
        ...
        attn_windows = self.attn(x_windows, prior, b, rpi=rpi_sa, mask=attn_mask)
...
class AttenBlocks(nn.Module):
    ...
    def __init__(self,
        ...
        self.SPB = Scale_aware_Prompt_Block(dim, dim, expansion_rate=mlp_ratio)
    ...
    def forward(self, x, prior, x_size, params):
        ## Scale_aware_Prompt_Block ## 
        x = self.SPB(x, x_size, params["scale"])

        for blk in self.blocks:
            x = blk(x, prior, x_size, params['rpi_sa'], params['attn_mask'])

        x = self.attn(x, x_size, params['rpi_oca'])

        if self.downsample is not None:
            x = self.downsample(x)
        return x

# Residual Attention Groups
class AG(nn.Module):
    """
    Args:
        dim (int): Number of input channels.
    ...
    def forward(self, x, prior, x_size, params):
        return self.patch_embed(
            self.conv(self.patch_unembed(self.residual_group(x, prior, x_size, params), x_size))) + x
...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.PFB = PFB(dim, heads, qkv_bias, embed_dim, LayerNorm_type)
        ...
        if self.upsampler == 'pixelshuffle':
            # for ArSR Implicit Neural Module
            self.upsample = INM(embed_dim)
    ...
    def forward_features(self, x, prior, scale):
        ...
        params = {'attn_mask': attn_mask, 'rpi_sa': self.relative_position_index_SA,
                  'rpi_oca': self.relative_position_index_OCA, "scale": scale}
        ...
        for layer in self.layers:
            ...
            x = self.pos_drop(x)
            x = layer(x, prior, x_size, params)
            x = self.norm(x)  # b seq_len c
            x = self.patch_unembed(x, x_size)

        return x

    def forward(self, x, prior, coord, cell, mod_pad_h=None, mod_pad_w=None, scale=None):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x, prior, modscale)) + x
            if scale is not None:
                x = x[:, :, 0:x.shape[-2] - mod_pad_h, 0:x.shape[-1] - mod_pad_w]
            x = self.upsample(x, prior, coord.flip(-1), cell)
