...
class DSF_DF2K_Dataset(data.Dataset):
    def __init__(  
                    self,
                    opt
                ):
        super(DSF_DF2K_Dataset, self).__init__()
        ...
        self.__preprocess__()
        self.num_images = len(self.indeices)
    ...
    def __preprocess__(self):
        ...
        print("processing DIV2K images...", self.div2k_root)
        temp_path   = os.path.join(div2khr_path,'*.%s'%(self.subffix))
        images      = glob.glob(temp_path)
        for item in images:
            data_path_list.append(item)

        # data_path_list = sorted(list(scandir(div2khr_path, recursive=True, full_path=True)))
    ...
    def __getitem__(self, index):
        ...
        w_lr    = self.l_ps
        w_hr    = round(w_lr * s)
        if hr_img.shape[-1] < 1200:
            x0      = random.randint(0, hr_img.shape[-2] - w_hr * 2)
            y0      = random.randint(0, hr_img.shape[-1] - w_hr * 2)
        else:
